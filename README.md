...
- **每日重训（学习）**：180 交易日滚动窗口，每期用截至当日的最新真实数据重新训练
  全部短期逐日模型与 VAR；训练区间、有效样本行数、重训时刻都写入报告"模型学习"面板，
  权重向新数据学习并与上一期 EMA 平滑，历史权重在 `factor_weights` 表逐期可追溯，
  报告展示每个因素本期相对上期的权重变化（pp）。
