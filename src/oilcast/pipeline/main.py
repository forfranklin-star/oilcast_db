    ...
    if anchor:
        ...
        if hist_dates:
            ...
            prev_tbl = pd.read_sql_query(
                ...
                conn, params=(prev_weight_date,))
            conn.close()
            prev_weights = prev_tbl.set_index("factor")["weight"] if not prev_tbl.empty else None
        weights = learn_factor_weights(X_w, y_w, available=factor_avail, prev_weights=prev_weights)
...
def _build_narratives(bundle, prices, current_meta, short_out, mid_out, long_out,
                     weights, events, bt, model_learning=None):
