...
def learning_narrative(ml: dict) -> str:
    ...
    if tm:
        ...
        seg.append(f"短期模型已于 {ml.get('retrained_at','')} 用截至 {first.get('train_end','')} "
                   f"的 {first.get('n_valid','?')} 行真实样本重训（{ml.get('rolling_window','?')} 交易日滚动窗口）")
    if isinstance(bt, dict) and bt.get("available"):
        ...
        seg.append(f"滚动样本外回测 {bt['n_origins']} 个原点，MAE {bt['mae_pct']}%、"
                   f"方向命中率 {bt['direction_accuracy']*100:.0f}%，相对随机游走基准"
                   f"（{bt['benchmark_mae_pct']}%）{beat}")
