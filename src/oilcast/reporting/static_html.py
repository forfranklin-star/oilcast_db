...
<div class="wrap">
 ...
 <div class="card">
   ...
   <div class="kv"><span>滚动训练窗口 / 累计运行期数</span><b>最近 {{ ml.rolling_window }} 个交易日 ｜ 第 {{ ml.n_runs }} 期</b></div>
   {% for t, tm in ml.train_meta.items() %}
   <div class="kv"><span>{{ report.names[t] }}短期模型真实训练样本</span>
     <b>{{ tm.train_start }} ~ {{ tm.train_end }}，有效 {{ tm.n_valid }} 行（取数 {{ tm.n_rows }} 行），逐日模型 {{ tm.n_steps }} 个</b></div>
   {% endfor %}
 </div>
 <div class="grid cards" style="margin-top:14px">
   ...
   <div class="card"><h3>滚动样本外回测（短期·{{ bt.horizon_td if bt.available else '—' }}交易日）</h3>
     ...
     <div class="kv"><span>平均绝对误差 MAE</span><b>{{ bt.mae_pct }}%</b></div>
     <div class="kv"><span>均方根误差 RMSE</span><b>{{ bt.rmse_pct }}%</b></div>
     <div class="kv"><span>方向命中率</span><b>{{ (bt.direction_accuracy*100)|round(0) }}%</b></div>
     <div class="kv"><span>随机游走基准 MAE</span><b>{{ bt.benchmark_mae_pct }}%</b></div>
     <div class="kv"><span>是否跑赢随机游走</span><b class="{{ 'down' if bt.mae_pct < bt.benchmark_mae_pct else 'up' }}">{{ '是' if bt.mae_pct < bt.benchmark_mae_pct else '否' }}</b></div>
